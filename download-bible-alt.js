// Alternative Bible Download Script
// Trying multiple sources to get complete KJV Bible
...
const POSSIBLE_URLS = [
    ...
    'https://raw.githubusercontent.com/scrollmapper/bible_databases/master/cross_references/json/kjv.json',
    'https://raw.githubusercontent.com/jburson/bible-data/master/kjv.json',
    'https://raw.githubusercontent.com/jburson/bible-data/main/kjv.json'
];

function downloadBible(url, index = 0) {
    if (index >= POSSIBLE_URLS.length) {
        console.error('❌ All download attempts failed. Trying alternative approach...');
        return;
    }
            ...
            res.on('end', () => {
                ...
                try {
                    ...
                    if (index + 1 < POSSIBLE_URLS.length) {
                        ...
                        console.error('\n❌ All sources failed. You may need to manually add Bible data.');
                    }
                }
            });
        } else {
...
function processBibleData(bibleJson) {
    ...
    if (bibleJson.books && Array.isArray(bibleJson.books)) {
        // Structure: { books: [{ name, chapters: [{ number, verses: [{ number, text }] }] }] }
        books = bibleJson.books;
    } else if (Array.isArray(bibleJson)) {
        // Structure: [{ name, chapters: [...] }]
        books = bibleJson;
    } else if (bibleJson.kjv && bibleJson.kjv.books) {
        // Nested structure
        books = bibleJson.kjv.books;
    } else {
...
// Start download
console.log('🚀 Starting Bible download from multiple sources...');
downloadBible();
